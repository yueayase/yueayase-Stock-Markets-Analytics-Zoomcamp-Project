...
class DataRepository:
  ...
  def fetch_tickers(self, min_date=None):
    ...
    for i,ticker in enumerate(tq):
      ...
      historyPrices['declining_moving_average'] = np.where(historyPrices['SMA20'] < historyPrices['SMA50'], 1, 0)
  ...
  def fetch_indexes(self, min_date=None):
    ...
    vix_MA10 = vix['Close'].rolling(10).mean()
    m9['CVR3VIX_buy'] = (vix['Low'] > vix_MA10) | (vix['Close'] >= 1.1*vix_MA10 ) | (vix['Close'] < vix['Open'])
    m9['CVR3VIX_sell'] = (vix['High'] < vix_MA10) | (vix['Close'] <= 0.9*vix_MA10) | (vix['Close'] > vix['Open'])
